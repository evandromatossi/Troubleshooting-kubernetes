...
WEBHOOK_URL="https://outlook.office.com/webhook/2788ebf2-d880-4de3-a917-7b2b7e8742be@5a86b3fb-4213-49cd-b4d6-be91482ad3c0/IncomingWebhook/120f69654def4207a6e4ed59748a9d49/57
2b7657-643c-495a-8ce5-5188a1c28f91"
...
prd-admin akspriv-pricing-prd-admin akspriv-retira-prd-admin akspriv-rh-prd-admin akspriv-roleta-prd-admin akspriv-viapag-prd-admin akspriv-viaunica-prd-admin; do
      ...
      ERRO_HPA=$(kubectl get hpa -A | grep -v NAME | awk "{if(\$7>\$5) print \"\tMinimo: \"\$5,\"\tMaximo: \"\$6,\"\tAtual: \"\$7,\"\tPorcentagem: \"\$4,\"\tDeployment: \"\$
1}")
...
function validacao_final() {
    echo "### Aguardando o tempo de 30 minutos terminar para chamar a proxima validação ###"
    # Convert format text teams.
      TITULO="Validacoes dos clusters AKS finalizado com sucesso"
      MESSAGE_AGUARDE="Aguarde 30 minutos para o proximo teste no ambiente"
...
################################## Inicio da Validação ###################################
function validacao_inicio() {
    echo "### Iniciando a validação ###"
    # Convert format text teams.
      TITULO="Iniciando as validacoes dos clusters AKS"
      MESSAGE_AGUARDE="Realizando validação"
      MESSAGE_PROXIMO_TESTE=$( echo ${MESSAGE_AGUARDE} | sed 's/"/\"/g' | sed "s/'/\'/g" )
      JSON="{\"title\": \"${TITULO}\",\"text\": \"${MESSAGE_PROXIMO_TESTE}\" }"
      curl -H "Content-Type: application/json" -d "${JSON}" "${WEBHOOK_URL}"
      #sleep 600
